...
<div class="bg-gradient-to-r from-blue-800 to-blue-900 py-16">
    <div class="container mx-auto px-4 max-w-7xl">
        <h1 class="text-4xl font-bold text-white mb-2">Prestasi Sekolah</h1>
        <nav class="text-white/60 text-sm"><a href="{{ route('home') }}" class="hover:text-white">Beranda</a> / Prestasi</nav>
    </div>
</div>
<div class="container mx-auto px-4 max-w-7xl py-12">
    {{-- Filters --}}
    <form method="GET" action="{{ route('achievements.index') }}" class="flex flex-wrap gap-3 mb-8">
        <select name="level" class="input-field w-auto" onchange="this.form.submit()">
        ...
        <select name="year" class="input-field w-auto" onchange="this.form.submit()">
            <option value="">Semua Tahun</option>
            @foreach($years as $year)
            <option value="{{ $year }}" {{ request('year') == $year ? 'selected' : '' }}>{{ $year }}</option>
            @endforeach
        </select>
        <button type="submit" class="btn-primary px-4"><i class="fas fa-filter mr-2"></i>Filter</button>
        @if(request()->hasAny(['level','category','year']))
        <a href="{{ route('achievements.index') }}" class="btn-outline px-4">Reset</a>
        @endif
    </form>
                ...
                <div>
                    <span class="text-xs font-medium px-2 py-0.5 rounded-full
                        {{ match($achievement->level) {
                            ...
                            'Provinsi' => 'bg-green-100 text-green-700',
                            default => 'bg-gray-100 text-gray-600'
                        } }}">{{ $achievement->level }}</span>
                    <div class="text-xs text-gray-400 mt-1">{{ $achievement->year }} &bull; {{ $achievement->category }}</div>
                </div>
            </div>
