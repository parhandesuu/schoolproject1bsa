...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-xs text-gray-500 mt-0.5">Kelola kategori untuk pengelompokan artikel berita</p>
    </div>
    @can('kategori.create')
    <a href="{{ route('admin.categories.create') }}" class="btn-primary"><i class="fas fa-plus"></i> Tambah Kategori</a>
    @endcan
</div>
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50/50">
                ...
                <td class="px-5 py-3.5 text-center">
                    <span class="inline-block w-6 h-6 rounded-full border border-gray-200 shadow-sm" style="background:{{ $cat->color ?? '#3b82f6' }}"></span>
                </td>
                <td class="px-5 py-3.5 text-center text-gray-600 font-semibold">{{ $cat->posts_count }}</td>
                <td class="px-5 py-3.5 text-center">
                ...
                <td class="px-5 py-3.5 text-center">
                    <div class="flex items-center justify-center gap-2">
                        @can('kategori.update')
                        <a href="{{ route('admin.categories.edit', $cat) }}" class="icon-btn-blue" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                        @endcan
                        @can('kategori.delete')
