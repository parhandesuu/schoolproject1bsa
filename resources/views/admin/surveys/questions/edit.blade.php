...
@section('title', 'Edit Pertanyaan Survei SKM')

@section('page-title', 'Edit Pertanyaan Kuesioner')

@section('content')
<div class="max-w-4xl space-y-6" x-data="{
    opt4: '{{ old('opt4_label', $surveyQuestion->opt4_label) }}',
    opt3: '{{ old('opt3_label', $surveyQuestion->opt3_label) }}',
    opt2: '{{ old('opt2_label', $surveyQuestion->opt2_label) }}',
    opt1: '{{ old('opt1_label', $surveyQuestion->opt1_label) }}',
    ...
    <div class="flex items-center gap-2 text-xs text-gray-500 mb-1">
        ...
        <i class="fas fa-chevron-right text-[9px] text-gray-400"></i>
        <span class="text-gray-800 font-semibold">Edit Pertanyaan</span>
    </div>

    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Edit Pertanyaan: {{ $surveyQuestion->title }}</h1>
            <p class="text-xs sm:text-sm text-gray-500 mt-0.5">Perbarui teks indikator, urutan, atau opsi skala jawaban untuk butir pertanyaan ini.</p>
        </div>
        ...
        <form action="{{ route('admin.survey-questions.update', $surveyQuestion) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $surveyQuestion->order) }}" min="1" required
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $surveyQuestion->code) }}" placeholder="Contoh: U1, U2, ..."
                ...
                <div>
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $surveyQuestion->icon) }}" placeholder="fas fa-clipboard-check"
                           class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 text-xs sm:text-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-100 transition-all">
                    @error('icon')
                        <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="question" rows="4" required
                          ...
                          class="w-full p-3.5 rounded-xl border border-gray-200 text-xs sm:text-sm focus:border-blue-600 focus:ring-2 focus:ring-blue-100 transition-all">{{ old('question', $surveyQuestion->question) }}</textarea>
                @error('question')
                    <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="pt-4 border-t border-gray-100 flex items-center justify-between">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $surveyQuestion->is_active) ? 'checked' : '' }} class="sr-only peer">
